export default function PollCard({ author, question, options }) {
    ...
    return (
        <div className="p-2 mb-4 border-b-1 border-zinc-600 w-full">
            <div className="w-100 flex flex-col gap-3">
                <h1 className="font-semibold text-xl">{question}</h1>
                {options.map((option) => (
                    <div key={option.id} className="border-1 p-3 rounded-lg hover:cursor-pointer transform hover:scale-102 transition-all duration-200">
                        ...
                        <p className="text-right mt-1 text-zinc-400">{totalvotes !==0 ? (option.votes / totalvotes) * 100 : 0}% Votes</p>
